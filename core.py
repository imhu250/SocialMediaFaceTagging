...
def val(image,vals):
	ncheck=0
	size = 4
	q="select user_id from users where login_id='%s'" %(vals)
	print(q)
	res=select(q)
	print(res)

	q="insert into images values(null,'%s','%s',curdate(),'pending')" %(res[0]['user_id'],image)
	id=insert(q)
	q1="insert into post values(null,'%s','%s')" %(id,res[0]['user_id'])
	insert(q1)
	...
	for f in faces:
		...
		print(val)
		if str(val)!="":
			if str(res[0]['user_id']) != str(val):
				q="insert into imageusers values(null,'%s','%s','pending')" %(id,val)
				print(q)
				insert(q)
			else:
				pass
